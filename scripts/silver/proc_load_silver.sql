...
BEGIN
	BEGIN TRY
		...
		prd_id,
		prd_key,
		REPLACE(SUBSTRING(prd_key,1,5),'-','_') cc_cat_id,
		SUBSTRING(prd_key,7,LEN(prd_key)) cc_prd_key,
		prd_nm,
		...
		prd_start_dt,
		prd_end_dt,
		LEAD(DATEADD(day,-1,prd_start_dt)) OVER(PARTITION BY prd_key ORDER BY prd_start_dt ASC) cc_prd_end_dt
		...
		CASE WHEN bdate > GETDATE()
			...
			ELSE bdate
		END bdate,
		CASE WHEN LOWER(TRIM(gen)) IN ('f','female')
			THEN 'Female'
			WHEN LOWER(TRIM(gen)) IN ('m','male')
			THEN 'Male'
			ELSE 'n/a'
		END gen
		...
		PRINT '----------------------------------------------------------------------';
		PRINT 'finished loading silver tables';
		PRINT '----------------------------------------------------------------------';
	END TRY
	BEGIN CATCH
		PRINT 'ERROR WHEN LOADING SILVER LAYER';
